...
function updateCpsxxq(){
	...
	if(checkCpsx() && checkStr()){
		if(confirm("真的确认提交吗？")){
			$.post("index.php?r=cpgl/UpdateCpsxxq",{id:id,pid:pid,cpsx:cpsx,str:str},function(data){
				if(data.res == "success"){
					alert("添加成功！");
					window.location.href="index.php?r=cpgl/GetCpsxxqHtml&id="+pid;
				}else{
					alert(data.mes);
				}
			},"json");
