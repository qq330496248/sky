$(function(){
	var cplb = $("#cplb").val();
	var hol = $("#hol").val();
	$.post("index.php?r=flbb/GetCplbtjbbChart",{cplb:cplb,hol:hol},function(data){
		getTable(data);	
	},"json");
});
	...
	if(data.result == "success"){
		...
		for(var i = 0; i < length; i += 2){
			var listInfo = '<tr class="singular">'
						...
						+'<td><font style="color:#FF0000">'+data.list[i]['qrNum']+'</font>/<font style="color:#0000FF">'+data.list[i]['qrMoney']+'</font></td>'
			...
			if(i < length - 1){
				listInfo += '<tr class="complex">'
						...
						+'<td><font style="color:#FF0000">'+data.list[i+1]['qrNum']+'</font>/<font style="color:#0000FF">'+data.list[i+1]['qrMoney']+'</font></td>'
		...
		var tfoot = '<tr>'
					...
					+'<td><font style="color:#FF0000">'+ data.totalQrNum +'</font>/<font style="color:#0000FF">'+ data.totalQrMoney +'</font></td>'
...
function getChart(str){
	...
	$.post("index.php?r=flbb/GetCplbtjbbChart",{cplb:cplb,day:str,endDate:endDate},function(data){
		getTable(data);	
	},"json");
}