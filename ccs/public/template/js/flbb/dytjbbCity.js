$(function(){
	...
	$.post("index.php?r=flbb/GetDytjbbCity",{beginDate:beginDate,endDate:endDate,pid:pid},function(data){
		getTable(data);
	},"json");
});
	...
	if(data.result == 'success'){
		...
		for(var i = 0; i < length; i += 2){
			var listInfo = '<tr class="singular">'
							+'<td><span>'+ data.list[i]['pro'] +'</span></td>'
					...
					//		+'<td><a target="_blank" style="color:#0000FF" href="public/template/grfUtils/data/xmlDYTJBB.php?pro='+data.list[i]['pro']+'&obd='+data.beginDate+'&oed='+data.endDate+'">报表查询</a></td></tr>';
							+'<td><a target="_blank" style="color:#0000FF" href="index.php?r=flbb/GetDytjbbChartHtml&pro='+data.list[i]['pro']+'">报表查询</a></td></tr>';
			if(i < length - 1){
				listInfo += '<tr class="complex">'
							+'<td><span>'+ data.list[i+1]['pro'] +'</span></td>'
					...
					//		+'<td><a target="_blank" style="color:#0000FF" href="public/template/grfUtils/data/xmlDYTJBB.php?pro='+data.list[i+1]['pro']+'&obd='+data.beginDate+'&oed='+data.endDate+'">报表查询</a></td></tr>';
							+'<td><a target="_blank" style="color:#0000FF" href="index.php?r=flbb/GetDytjbbChartHtml&pro='+data.list[i+1]['pro']+'">报表查询</a></td></tr>';
			}
...
function getDytjbb(){
	...
	var type = $("#type").val();
	var pid = $("#pID").val();
	$.post("index.php?r=flbb/GetDytjbbCity",{beginDate:beginDate,endDate:endDate,dept:dept,khyx:khyx,type:type,pid:pid},function(data){
		getTable(data);
	},"json");
}
...
function selectDay(str){
	...
	var type = $("#type").val();
	var pid = $("#pID").val();
	$.post("index.php?r=flbb/GetDytjbbCity",{day:str,dept:dept,type:type,pid:pid},function(data){
		getTable(data);
	},"json");
}
...
function changeDay(str){
	...
	var type = $("#type").val();
	var pid = $("#pID").val();
	$.post("index.php?r=flbb/GetDytjbbCity",{beginDate:beginDate,endDate:endDate,day:str,dept:dept,type:type,pid:pid},function(data){
		getTable(data);
	},"json");
}
...
function changeType(str){
	...
	var dept = $("#dept").val();
	var pid = $("#pID").val();
	$.post("index.php?r=flbb/GetDytjbbCity",{beginDate:beginDate,endDate:endDate,type:str,dept:dept,pid:pid},function(data){
		getTable(data);
	},"json");
}