...
$(function(){
	 ...
	 * @date 2015-11-04
	 */
  $.post('index.php?r=khgl/Subordinate',function(data){
      listData(data);
    //全选(全不选)特效
...
function listData(data){
  ...
  if(data.result == 'success'){
    ...
    $('body').on('click','#batchArragin',function(){
        ...
        var a= zgtsgh.split(":");
        var ensure = confirm('确定要把选中的客户资料转给工号:'+a[0]+'吗？');
        if(ensure){
            $.post('index.php?r=khgl/getTurnOutCustomer',{orderno :orderno,zgtsgh:zgtsgh},function(data){
                if(!data){
                    alert('转出失败，请重新操作！');
                }
                if(data.res == 'success'){
                    alert(data.msg);
                  window.location.href="index.php?r=khgl/GetXskhzlHtml&xskhzl.html";
                }
                if(data.res == 'error'){
                    alert(data.msg);
                    return;
 ...
 * @date 2016-02-01
 */
function SubordinateClientData(sign,page,psize){ 
   ...
   if(sign == 1){
      ...
      $.get('index.php?r=khgl/Subordinate',{khname :khname,khphone :khphone,khgmcp:khgmcp,khnlsq:khnlsq,khnlsz:khnlsz,khdj:khdj,
    xfjed:xfjed,xfjex:xfjex,zcfs:zcfs,khly:khly,nsr:nsr,gjbq:gjbq,gmcs:gmcs,zxgjq:zxgjq,zxgjz:zxgjz,zxzdq:zxzdq,zxzdz:zxzdz,zxqsq:zxqsq,zxqsz:zxqsz,zxjsq:zxjsq,zxjsz:zxjsz,khzcsjq:khzcsjq,khzcsjz:khzcsjz,khxb:khxb,khsr:khsr,khsf:khsf,khgsgh:khgsgh,city:city,area:area,sign:sign,page:page,psize:psize},function(data){
          if(!data){
              return;
          }
      ...
      $.ajax({
          type: "get",
          url: "index.php?r=khgl/Subordinate",
          async: true,
          data: {khname :khname,khphone :khphone,khgmcp:khgmcp,khnlsq:khnlsq,khnlsz:khnlsz,khdj:khdj,xfjed:xfjed,xfjex:xfjex,zcfs:zcfs,khly:khly,nsr:nsr,gjbq:gjbq,gmcs:gmcs,zxgjq:zxgjq,zxgjz:zxgjz,zxzdq:zxzdq,zxzdz:zxzdz,zxqsq:zxqsq,zxqsz:zxqsz,zxjsq:zxjsq,zxjsz:zxjsz,khzcsjq:khzcsjq,khzcsjz:khzcsjz,khxb:khxb,khsr:khsr,khsf:khsf,khgsgh:khgsgh,city:city,area:area,sign:sign},
 ...
 * @date 2015-11-03
 */
    $('#StartSearchClient').on('click',function(){
       var page = '';
       var psize = '';
       var sign = 0; //查询标识
       SubordinateClientData(sign,page,psize);
	});
...
//表格表头，按照顺序倒序，加入箭头图标
  function changeStr(str){
    ...
    $.post("index.php?r=khgl/Subordinate",{page:page,psize:psize,order:order,sequence:sequence},function(data){
      listData(data);
      //全选(全不选)特效
      checkAll($('#checkall'),$('.checkbox'));
    },"json");
