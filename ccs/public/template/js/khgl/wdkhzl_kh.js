...
$(function(){
	 ...
	 * @date 2015-10-30
	 */
	$.post('index.php?r=khgl/GetMyClient',function(data){
		listData(data);
		//全选(全不选)特效
...
$('body').on('click','#batchArragin',function(){
    ...
    if(ensure){
        $.post('index.php?r=khgl/getTurnOutCustomer',{orderno :orderno,zgtsgh:zgtsgh},function(data){
            ...
            if(data.res == 'success'){
                alert(data.msg);
              window.location.href="index.php?r=khgl/GetWdkhzlHtml&wdkhzl.html";
            }
            if(data.res == 'error'){
                alert(data.msg);
                return;
...
function MyClientData(sign,page,psize){ 
   ...
   if(sign == 1){
   		...
   		$.get('index.php?r=khgl/GetMyClient',{khname :khname,khphone :khphone,khgmcp:khgmcp,khnlsq:khnlsq,khnlsz:khnlsz,khdj:khdj,
   	xfjed:xfjed,xfjex:xfjex,zcfs:zcfs,khly:khly,nsr:nsr,gjbq:gjbq,gmcs:gmcs,zxgjq:zxgjq,zxgjz:zxgjz,zxzdq:zxzdq,zxzdz:zxzdz,zxqsq:zxqsq,zxqsz:zxqsz,zxjsq:zxjsq,zxjsz:zxjsz,khzcsjq:khzcsjq,khzcsjz:khzcsjz,khxb:khxb,khsr:khsr,khsf:khsf,city:city,area:area,sign:sign,page:page,psize:psize},function(data){
   		...
   		$.ajax({
            type: "get",
            url: "index.php?r=khgl/GetMyClient",
            async: true,
 ...
 */
 $('body').on('click','#exportExcel',function(){
	...
	function changeStr(str){
		...
		$.post("index.php?r=khgl/GetMyClient",{page:page,psize:psize,order:order,sequence:sequence},function(data){
			listData(data);
			//全选(全不选)特效
			checkAll($('#checkall'),$('.checkbox'));
		},"json");
